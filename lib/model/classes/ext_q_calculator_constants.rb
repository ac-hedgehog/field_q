...
push \t- положить элемент поля в стек
pop  \t- взять элемент поля из стека
stek \t- показать текущее состояние стека
...
-  \t- поменять знак элемента на противоположный
...
==================================================
  ...
  COMMANDS_MESSAGES = {
    ...
    :push   => 'Введите элемент поля:',
    :uses   => 'Использовать результат в дальнейших вычислениях? (y/n):'
  }
end